            ...
            <div className='w-full flex flex-col lg:flex-row p-2 gap-4 md:gap-10'>
              <div className=' flex flex-col gap-4 w-full md:min-w-[60%]    h-[200px] overflow-x-hidden  p-4'>
               {cartData.map((item,index)=>(
                 <div className=' relative flex flex-col md:flex-row justify-between items-center border rounded-md shadow-xl px-2 md:px-6 py-2 md:py-4'>
                 <p className=' absolute -top-2 md:top-1/2 md:-left-3  bg-gray-400 rounded-full p-1'>   <RiDeleteBin5Fill size={16} onClick={() => removeCartItem(item.id)} /></p>
                 <div className=' flex flex-row items-center gap-4'>
                   <Image src={CartImg} width={70} height={50} alt='' className=' w-20 h-28 py-2' />
                   <div className='flex flex-col text-sm font-semibold'><span>{item.title}</span><span>{item.size}</span></div>
                 </div>
                 <div className='flex flex-row gap-4 md:gap-6'>
                   ...
                   <span className='text-md font-semibold'>${calculateTotalPrice(item.price, item.quantity)}</span>
                 </div>
              ...
              <div className=' flex flex-col gap-2 w-full md:min-w-[30%]'>
                ...
                <Link href="/cart" className='bg-white hover:bg-[#132842] border-2 flex  justify-center w-full py-3 text-black hover:text-white shadow-gl rounded-3xl text-sm'>
                  VIEW CART
                </Link>
                <Link href="/products" className='bg-white hover:bg-[#132842] border-2 flex  justify-center w-full py-3 text-black hover:text-white shadow-gl rounded-3xl text-sm'>
                  CONTINUE SHOPPING
                </Link>
                ...
                <Link href="/checkouts" className='bg-gray-300  border-2 flex  justify-center w-full py-3 text-black shadow-gl rounded-3xl text-sm'>
                  PROCEED TO CHECKOUT
                </Link>
              </div>
            </div>
