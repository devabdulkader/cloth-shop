...
const QuickViewWishlistModal: React.FC<QuickViewWishlistModalProps> = ({
  onClose,
  wishlistItem,
}) => {
  // Move hooks to top level
  const [selectedSize, setSelectedSize] = useState<string>(
    wishlistItem?.sizes[0] || ""
  );
  const [quantity, setQuantity] = useState<number>(wishlistItem?.quantity || 1);
    ...
    <>
      <div className="fixed top-0 left-0 w-screen h-screen z-50 bg-black/50 backdrop-blur-sm">
        <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-[420px] sm:max-w-[600px] lg:max-w-[984px] p-4 max-h-[90vh] overflow-auto">
          <div className="w-full flex flex-col md:flex-row justify-between gap-2  bg-white shadow-md  rounded-md  overflow-hidden relative p-4">
            ...
            <div className="min-w-full flex flex-col gap-2 md:min-w-[48%] ">
              ...
              <p className="flex flex-row gap-8 text-sm font-normal">
                <span>SKU:</span> <span>{wishlistItem.sku}</span>
              </p>
              <p className="flex flex-row gap-8 text-sm font-normal">
                <span>Category:</span>{" "}
                ...
                <div className="flex space-x-2 mt-5">
                  {wishlistItem.sizes.map((size) => (
                    <button
                      ...
                      className={`border border-gray-300 rounded text-sm px-4 py-2 text-gray-700 ${
                        selectedSize === size
                          ? "bg-black text-white"
                          : "bg-gray-200"
                      }`}
          ...
          <div
            onClick={onClose}
            className="absolute top-4 right-4 text-black p-3 bg-slate-100 shadow-xl rounded-full cursor-pointer"
          >
